...
class ChessView:
    ...
    def draw_board(self):
        ...
        for (x, y) in all_pieces.keys():
            ChessView.piece_images[x, y] = tkinter.PhotoImage(file=all_pieces[x, y].get_image_path())
            ChessView.can.create_image(real_position(x), real_position(y), image=ChessView.piece_images[x, y])

        if self.chessboard.selected_pieces:      # 有选中的棋子时
            for (x, y) in self.chessboard.selected_pieces.findAll(self.chessboard):       # 找出所有可以走棋的位子

                self.can_move_img.append(tkinter.PhotoImage(file="images/OOS.gif"))
                self.can.create_image(real_position(x), real_position(y), image=self.can_move_img[-1])
